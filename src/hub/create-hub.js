...
function createHub({
  ...
  maxPendingSockets   = positiveFinite(maxPendingSockets,   DEFAULT_MAX_PENDING_SOCKETS,   'maxPendingSockets');
  keepaliveIntervalMs = positiveFinite(keepaliveIntervalMs, DEFAULT_KEEPALIVE_INTERVAL_MS, 'keepaliveIntervalMs');

  // 0 means "disabled" for these two
  replayWindowMs = nonNegFinite(replayWindowMs, DEFAULT_REPLAY_WINDOW_MS, 'replayWindowMs');
  helloTimeoutMs = nonNegFinite(helloTimeoutMs, DEFAULT_HELLO_TIMEOUT_MS, 'helloTimeoutMs');
  ...
  /**
   * Send an envelope to a peer's socket. Returns `true` on success,
   * `false` on any drop (closed socket, missing signing key, backpressure
   * cap exceeded, or `ws.send()` throwing).
   *
   * Callers that need the generated message id can pass `id` in
   * themselves (everywhere except RPC responses already does so for
   * correlation purposes); the return value is intentionally boolean
   * to make backpressure / drop checks unambiguous.
   */
  ...
  function attach(ws, req) {
    ...
    const ctx = {
      ...
      send, publishPeers, dropSubscriptionsFor,
